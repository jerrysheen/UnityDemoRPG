...
half HeroNormalDistribution0(float roughness, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS)
{
...
half HeroDirectBRDFSpecular(BRDFData brdfData, half3 normalWS, half3 lightDirectionWS, half3 viewDirectionWS)
{
    float3 lightDirectionWSFloat3 = float3(lightDirectionWS);
    float3 halfDir = SafeNormalize(lightDirectionWSFloat3 + float3(viewDirectionWS));
    ...
    float D = 0.75 * HeroNormalDistribution0(brdfData.roughness, normalWS, lightDirectionWS, viewDirectionWS);
    D += 0.25 * HeroNormalDistribution0(brdfData.roughness2, normalWS, lightDirectionWS, viewDirectionWS);
...
float ComputeWrappedDiffuseLighting2(float NdotL, float w)
{
    w=cos(PI/2 - PI/12);
...
float3 Fresnelschlick4Skin(float VoH, float3 rF0)
{
    VoH=1-VoH;
    float exponential = MPow5( VoH);
    //皮肤使用0.028
    return exponential + 0.028  * (1 - exponential);
}
float D_GGXaniso(float ax, float ay, float NoH, float3 H, float3 x, float3 y)
{
    ...
    // return 1 / ( _PI * ax*ay * d*d );
    float XoH = dot( x, H )/ax;
    float YoH = dot( y, H )/ay;
    float d = XoH*XoH  + YoH*YoH  + NoH*NoH;
    return  0.25*rcp( 3.141593 * ax * ay * max( d*d, 1e-6 ) );
}

//geometric / visibility
float G_Geometryaniso(float a, float NoL, float NoH)
{
    float k = a * 0.5;
    float G_SmithL = NoL * (1.0 - k) + k;
    float G_SmithV = NoH * (1.0 - k) + k;
    return  0.25 * rcp( G_SmithL * G_SmithV );
}
