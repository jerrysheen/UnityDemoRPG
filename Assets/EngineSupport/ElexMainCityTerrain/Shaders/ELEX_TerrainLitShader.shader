...
{
    // 23-8-16: 性能表现效果分级：
    // 在不同的机型段，multi_compile来切换性能，需要在运行时切换。
    // 需要切分的功能有一下几个：
    // 1. 混合层数， 这个在制作时已经确定，目前分为6层和9层两种。
    // 2. 世界法线开关
    // 3. 基于高度图的混合开关
    // 4. 更为简单的光照算法
    // 第一点只需要shader_feature来控制， 后面三点可以通过简单定义一个LOW_QUALITY关键词，然后通过这个关键词去控制要开还是关某些feature
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            #include "./Include/MainCityTerrainLitInput.hlsl"
            #include "./Include/MainCityTerrainLitForward.hlsl"
